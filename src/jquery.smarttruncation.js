 ...
 *
 * Modified by Neil Monroe
 * 2013.11.14 - Add support for a fixed number of lines to calculate against.
 *
 * USAGE:
 *
...
(function ($) {
    $.fn.smartTruncation = function (options) {
        ...
        return $(this).each(function () {
            ...
            var $wrapper = $this
              .wrapInner('<span class="smarttruncation-wrapper"></span>')
              .find('span').css({
                  'whiteSpace': 'nowrap'
              });
            ...
            var update = function () {
                ...
                var lineLength = ($this.width() - 3 * sizes['.']) * settings.lines * settings.resizeFactor;
                var diff = lineLength - outerWidth;
                var safety;
                ...
                if (diff <= 0) {
                    ...
                    if (settings.truncateCenter) {
                        ...
                        if ($wrapper.width() > lineLength) {
                            while ($wrapper.width() > lineLength) {
                            ...
                            if ($wrapper.width() + safety < lineLength) {
                                while ($wrapper.width() + safety < lineLength) {
                                    lengthLeft < lengthRight ? lengthLeft++ : lengthRight++;
                                    $wrapper.text(
                                      $.trim(left.slice(0, lengthLeft).join(""))
                                        + "..."
